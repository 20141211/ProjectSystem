 ...
 * 
 * @param {Object}the
 *            checkbox unique id: c+@objName+id
 * @param {Object}the
 *            checkbox's parent id
 * @param {Object}the
 *            display name
 * @param {Object}checked
 *            or not
 * @param {Object}the
 *            navigation url if not null
 * @param {Object}the
 *            window.status if onhover and useStatusText:true
 * @param {Object}the
 *            target browser window if url not null
 * @param {Object}the
 *            display icon
 * @param {Object}the
 *            node icon if it is opened
 * @param {Object}the
 *            open status
 * @member toString to Json format string
 */
function Node(id, pid, name, check, value, formName, checkBoxTitle,url, title, target, icon,
		iconOpen, open) {
	this.id = id;
	this.check = check;
	this.pid = pid;
	...
	this.toString = function() {
		return "[" + "id:" + this.id + ",pid:" + this.pid + ",name:"
				+ this.name + "]";
	};
}
// Tree object 枚举
function dTree(objName) {
	...
	this.icon = {
		root : "dtreeimg/base.gif",
		folder : "dtreeimg/folder.gif",
		folderOpen : "dtreeimg/folderopen.gif",
		node : "dtreeimg/page.gif",
		empty : "dtreeimg/empty.gif",
		line : "dtreeimg/line.gif",
		join : "dtreeimg/join.gif",
		joinBottom : "dtreeimg/joinbottom.gif",
		plus : "dtreeimg/plus.gif",
		plusBottom : "dtreeimg/plusbottom.gif",
		minus : "dtreeimg/minus.gif",
		minusBottom : "dtreeimg/minusbottom.gif",
		nlPlus : "dtreeimg/nolines_plus.gif",
		nlMinus : "dtreeimg/nolines_minus.gif"
	};
	this.obj = objName;
	this.aNodes = [];
	this.aIndent = [];
...
dTree.prototype.test = function() {
	for ( var n = 0; n < this.aNodes.length; n++) {
		alert(this.aNodes[n].name + ":" + this.aNodes[n].check);
	}
};

// Adds a new node to the node array
dTree.prototype.add = function(id, pid, name, check, value, formName,checkBoxTitle, url,
		title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, check, value,
			formName,checkBoxTitle, url, title, target, icon, iconOpen, open);
};
...
// Creates the tree structure root new Node(-1)
dTree.prototype.addNode = function(pNode) {
	...
	for (n; n < this.aNodes.length; n++) {
		...
		if (this.aNodes[n].pid == pNode.id) {
			var cn = this.aNodes[n];//找到权限管理的节点Node
			cn._p = pNode;//父级元素 
			cn._ai = n;//存储时的位置
			cn.pNode = pNode;//父级元素
...
dTree.prototype.checkParentNode = function(node, check) {
	while (node.pNode) {
		var pCheck = document.getElementById(("c" + this.obj) + node.pNode.id);
		if (pCheck) {
			pCheck.checked = check;
			node.pNode.check = check;
		}
		node = node.pNode;
	}
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if (node.check && node.check != "") {
			...
			str += "<input  name='"
					+ (node.formName != undefined ? node.formName : this.obj)
					+ "' id='c" + this.obj + node.id
					+ "' onclick=\"checkNode('" + node.id
					+ "');\" type='checkbox' "
					+ (node.check === "true" ? "checked='checked'" : "")
					+ " value='" + node.value + "'title='"+(node.checkBoxTitle != undefined ? node.checkBoxTitle : this.obj)+"'/>";
		} else {
